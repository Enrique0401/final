document.addEventListener("DOMContentLoaded", () => {
    ...
    const subfiloSelect = document.getElementById("Subfilo");
    const filoSelect = document.getElementById("Filo");
    ...
    const applyFilters = () => {
        ...
        const selectedSubfilo = subfiloSelect.value;
        const selectedFilo = filoSelect.value;
        ...
        animales.forEach(animal => {
            ...
            const animalSubfilo = animal.dataset.subfilo;
            const animalFilo = animal.dataset.filo;
            ...
            const matchesFilo = selectedFilo === "" || animalFilo === selectedFilo;
            ...
            if (matchesSearch && matchesSubfilo && matchesFilo && 
                ...
                if (animal.parentElement.tagName === 'A') {
                    animal.parentElement.style.display = "block";
                }
            } else {
                animal.style.display = "none";
                
                // Ocultar también el contenedor padre si es un <a>
                if (animal.parentElement.tagName === 'A') {
                    animal.parentElement.style.display = "none";
                }
            }
        });
    };
    ...
    subfiloSelect.addEventListener("change", applyFilters);
    filoSelect.addEventListener("click", applyFilters);
    ...
    const getParametrosURL = () => {
        ...
        return {
            subfilo: params.get('Subfilo'),
            filo: params.get('Filo'),
    ...
    const aplicarFiltrosDesdeURL = () => {
        const filtros = getParametrosURL();

        if (filtros.subfilo) subfiloSelect.value = filtros.subfilo;
        if (filtros.filo) filoSelect.value = filtros.filo;
